...
export default function VerificationPipelineStepper({ activeStep = 5 }: Props) {
  ...
  return (
    <div className="p-4 sm:p-6 rounded-2xl bg-white dark:bg-slate-900/80 border border-slate-200 dark:border-slate-800 shadow-sm">
      {/* Header */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 pb-4 sm:pb-5 mb-4 sm:mb-5 border-b border-slate-100 dark:border-slate-800/80">
        <div>
        ...
        <div className="self-start sm:self-auto flex items-center gap-1.5 px-3 py-1 rounded-full text-[11px] sm:text-xs font-semibold bg-emerald-50 dark:bg-emerald-950/60 text-emerald-600 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-500/30">
          <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></span>
          Pipeline Running Live
        </div>
      </div>

      {/* Stepper with scroll wrapper for mobile */}
      <div className="overflow-x-auto pb-2 scrollbar-none">
        <div className="min-w-[480px] relative flex items-center justify-between pt-2 px-2">
